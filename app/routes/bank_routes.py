...
@bank_routes.post("/", response_description = 'ADMINS only: Create bank', status_code = 201, response_model = BankDetailsModel)
async def add_bank(authorization: str = Header(...), data: CreateBankModel = Body(...)):
    token = authorization.split(" ")[1]
    return await create_bank(token, data)

@bank_routes.put("/{bank_id}", response_description = 'ADMINS only: Update bank', status_code = 200, response_model = BankDetailsModel)
...
@bank_routes.delete("/{bank_id}", response_description = 'ADMINS only: Delete bank', status_code = 200)
