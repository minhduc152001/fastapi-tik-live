...
async def live_connect(live_connect: LiveConnectBody, background_tasks: BackgroundTasks, authorization: str = Header(...)):
    token = authorization.split(" ")[1]
    tiktok_id = live_connect.tiktok_id
    return await connect_live(token, tiktok_id, background_tasks)