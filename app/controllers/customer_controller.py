...
from app.models.customer_model import CustomerUpdate, DeleteRequest, AddLocalCustomer
...
from app.services.customer_service import update_customer_service, get_local_customer_service, \
    get_global_customer_service, remove_customer_elements_service, add_local_customer_service
...
async def get_local_customer(token: str, customer_user_id: str, from_live_of_tiktok_id: str):
    ...
    return await get_local_customer_service(customer_user_id, from_live_of_tiktok_id,  str(user.get('_id')))
...
async def remove_customer_elements(token: str, request: DeleteRequest, customer_user_id: str, from_live_of_tiktok_id: str):
    user = await auth_middleware(token = token)
    return await remove_customer_elements_service(request, customer_user_id, from_live_of_tiktok_id, str(user.get('_id')))

async def create_local_customer(token: str, data: AddLocalCustomer):
    user = await auth_middleware(token = token)
    return await add_local_customer_service(str(user.get('_id')), data)